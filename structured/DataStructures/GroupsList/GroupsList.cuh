// This code block prevents this header file from being included more than once by the preprocessor
...
namespace structured{
    ...
    class groupsList{
        ...
        public:
            ...
            groupsList(std::vector<std::vector<int>> groups,bool sym = false){ init(groups,sym); }

            // Constructor that initializes the group list from a nested vector of groups.
            groupsList(std::vector<int> id,
                       std::vector<std::vector<int>> groups, bool sym = false){ init(id,groups,sym); }
