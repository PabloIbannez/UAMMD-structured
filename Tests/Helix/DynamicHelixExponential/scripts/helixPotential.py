...
def analyticPotential():
    ...
    # The potential is Ub =
    # (exp(-Kb(1.0-cos(pi*r/rc)))-A)/(B-A) where A = exp(-2Kb) and B = 1.0 for r <= rc
    # 0.0 for r > rc

    Ub = (sp.exp(-Kb_sym*(1.0-sp.cos(3.14159265*r/rc_sym)))-sp.exp(-2.0*Kb_sym))/(1.0-sp.exp(-2.0*Kb_sym))
    Ub = sp.Piecewise((Ub,r<=rc_sym),(0.0,r>rc_sym))
