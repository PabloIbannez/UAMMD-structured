...
sumPairForces = np.zeros([len(totalForces),3])
...
if (maxError<threshold):
    print(f"TEST PAIRWISE_FORCES NONBONDED PASSED: The maximum error is {maxError}")
else:
    print(f"TEST PAIRWISE_FORCES NONBONDED FAILED: The maximum error is {maxError}")
