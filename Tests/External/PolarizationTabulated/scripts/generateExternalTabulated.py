...
maxPol = inputData["maxPolarizability"]
...
sim["state"]={}
sim["state"]["labels"]=["id","position","polarizability"]
sim["state"]["data"]=[]
for i in range(N):
    x,y,z = np.random.rand(3)*np.array([Lx-2*diffx,Ly-2*diffy,Lz-2*diffz])-np.array([(Lx-2*diffx)/2,(Ly-2*diffy)/2,(Lz-2*diffz)/2])
    #A random number between -maxPol and maxPol
    p = np.random.rand()*2*maxPol-maxPol
    sim["state"]["data"].append([i,[x,y,z],p])
...
sim["topology"]["forceField"]["ExTab"]["type"]=["External","PolarizationTabulated"]
