...
exponentialParamsLabels = sim["topology"]["forceField"]["exponential"]["patchesTopology"]["forceField"]["exponential"]["labels"]
exponentialParamsData   = sim["topology"]["forceField"]["exponential"]["patchesTopology"]["forceField"]["exponential"]["data"][0]

exponentialParams = {l:d for l,d in zip(exponentialParamsLabels, exponentialParamsData)}

Eb = exponentialParams["E"]

Kb  = exponentialParams["K"]
rc  = exponentialParams["rc"]

connections = sim["topology"]["forceField"]["exponential"]["patchesState"]["data"]
...
for id_,info in partInfo.items():
    e = float(info["exponentialEnergy"])
    ...
    f = np.asarray([info["exponentialForceX"], info["exponentialForceY"], info["exponentialForceZ"]])
    t = np.asarray([info["exponentialTorqueX"], info["exponentialTorqueY"], info["exponentialTorqueZ"]])
    totalEnergy += e
    totalForce  += f
