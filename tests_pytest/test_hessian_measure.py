...
def create_state_and_topology_nonbonded_clashed():
    ...
    topology = {
        "forceField": {
            "nl":{"type":["VerletConditionalListSet","all"],
                  "parameters":{"cutOffVerletFactor": 1.2}},
            "Bond": {
                "type": ["NonBonded", "Clashed"],
                "parameters": {"lambda":lambd, "gamma":gamma,
                               "condition":"all"},
            }
        },
        "structure": {
    ...
    topology = {
        "forceField": {
            ...
            "Bond": {
                ...
                "parameters": {"dielectricConstant":epsilon, "debyeLength":lambd,
                               "condition":"all", "cutOffFactor":2},
            }
        },
        "structure": {
            "labels": ["id", "type"],
    ...
    topology = {
        "forceField": {
            ...
            "Bond": {
                ...
                "data":[["A", "A", eps, sig]],
                "parameters": {"dielectricConstant":dielConst,
                               "debyeLength":debLength,
                               "condition":"all",
                               "cutOffNPFactor":2**(1./6.),
                               "cutOffDHFactor":2**(1./6.)
                               },
            }
        },
        "structure": {
    ...
    topology = {
        "forceField": {
            ...
            "Bond": {
                ...
                "parameters": {"condition":"all", "cutOffFactor":2**(1./6)},
                "labels":["name_i", "name_j", "epsilon", "sigma"],
...
def test_hessian_row_sums_are_zero(tmp_path, hessian_mode, interactor):
    ...
    for i in range(nRows):
        ...
        assert np.allclose(net_sum, 0.0, atol=tol), \
